...
namespace ScadaCenterServer.Pages
{
    ...
    /// 
    /// </summary>
    public partial class ScadaBatchCommandTaskForm : DockForm
    {

      
        private IO_SERVER Server = null;
        private BatchCommandTaskModel   BatchCommandTask = null;
        public void SetBatchCommandTaskModel(BatchCommandTaskModel batchCommand   )
        {
            BatchCommandTask = batchCommand;
            this.search.SetTrainSelected(batchCommand);
            this.Server = this.search.Server;
        }

        public ScadaBatchCommandTaskForm()
        {
            InitializeComponent();
        ...
        public ScadaBatchCommandTaskForm(Mediator m):base(m)
        {
            InitializeComponent();
            this.Load += RealAlarmQueryWorkForm_Load;
        ...
        private void Search_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (this.search.Server != null)
            {
                this.Text = this.search.Server.SERVER_NAME;
                    if(this.search.BatchCommandTask != null)
                {
                    this.Text += this.search.Server.ToString()+"//" + this.search.BatchCommandTask.ToString();
                }




                this.BatchCommandTask = this.search.BatchCommandTask;
                this.Server = this.search.Server;
        ...
        private async void ReadBatchCommandTaskHistory()
        {
            try
            {
                ...
                //获取读取历史数据
                InfluxDBHistoryData resultData = await IOCenterManager.QueryFormManager.ReadHistoryBatchCommandTask(this.search.Server, this.search.BatchCommandTask, this.search.StartDate, this.search.EndDate,this.ucPagerControl.PageSize, this.ucPagerControl.PageIndex);
                ...
                if (resultData.Seres.Count() > 0)
                {
                    ...
                    {   
                        ...
                        int field_tasktype = s.Columns.IndexOf("field_tasktype");
                        if (field_tasktype >= 0)
                        {
                            lvi.SubItems.Add(InfluxDbManager.GetInfluxdbValue(s.Values[i][field_tasktype]));
                        }
                        else
                        {
                            lvi.SubItems.Add("");
