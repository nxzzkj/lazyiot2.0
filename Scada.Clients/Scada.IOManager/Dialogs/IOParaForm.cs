...
namespace IOManager.Dialogs
{
    public partial class IOParaForm : BasicDialogForm
    {
        ...
        public   void InitForm()
        {
            ...
            for(int i=0;i< IOManagerUIManager.mediator.IOParaForm.IOListView.ListView.Items.Count;i++)
            {
                listParas.Items.Add(IOManagerUIManager.mediator.IOParaForm.IOListView.ListView.Items[i].SubItems[1].Text);
            }
            //加载设备驱动
            ...
            if(driver!=null)
            {
                DeviceKernel = IOManagerUIManager.CreateDeviceDrive(driver);
        ...
        private  void wizardTabControl_ButtonOK(object sender, EventArgs e)
        {
            try
            {
                ...
                if( Para.IO_ID==null|| Para.IO_ID == "")//新建
                {
                    ...
                    Para.IO_ID = GUIDToNormalID.GuidToLongID().ToString();
                       IOListViewItem lvi = new IOListViewItem(Para);
                    this.Device.IOParas.Add(Para);
                    IOManagerUIManager.mediator.IOParaForm.AddListViewItem(lvi);
                    IOManagerUIManager.mediator.IOLogForm.AppendText(IOManagerUIManager.mediator.IOParaForm.IOListView.IOPath + " 创建IO点" + Para.IO_NAME + "成功!");
                }
                else
                {
                    ...
                    IOListViewItem lvi=  IOManagerUIManager.mediator.IOParaForm.GetListViewItem(Para);
                    if(lvi!=null)
                    {
                        lvi.ResetSubItems();
                        IOManagerUIManager.mediator.IOLogForm.AppendText(IOManagerUIManager.mediator.IOParaForm.IOListView.IOPath + " 修改IO点" + Para.IO_NAME + "成功!");
                    }
            ...
            {
                  IOManagerUIManager.DisplayException(emx);
            }
        }
