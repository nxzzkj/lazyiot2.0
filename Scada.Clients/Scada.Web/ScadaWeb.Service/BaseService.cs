...
namespace ScadaWeb.Service
{
    public abstract class BaseService<T> where T : class, new()
    {
        ...
        {
            long id = BaseRepository.Insert(model);
     
            return id > 0 ? true : false;
        }
        public long InsertReturnId(T model)
        {
            long id = BaseRepository.Insert(model);
      
            return id ;
        }
        public bool InsertAndId(T model)
        {
            long id = BaseRepository.InsertAndId(model);

            return id > 0 ? true : false;
        }
        public bool Insert(T model,out long id)
        {
