    ...
    this.name = "宁夏众智科技有限公司Lazy Scada系统";//公共属性
        this.refreshtime = "2s";//定SVG_IOLabelShape义每10秒刷新一次
        this.OpenMenu = function (menuid) {
            $("#popMenu").attr("xlink:href", "");
            $("#popMenu").attr("visibility", "hidden");
            var items = $("g[data-filter='menu']");
            for (var i = 0; i < items.length; i++) {
                var submenuid = $("#" + items[i].id + ">g[data-filter='menuitems']")[0].id;
                if (submenuid != menuid) {
                    $("#" + submenuid).attr("visibility", "hidden");
                    $("#" + submenuid).css("display", "none");

                }

            }
            if ($("#" + menuid).attr("visibility") == "visible") {
                $("#" + menuid).attr("visibility", "hidden");
                ...
                $("#" + menuid).attr("visibility", "visible");
                $("#" + menuid).css("display", "block");
         
                $("#popMenu").attr("xlink:href", "#" + menuid);
                $("#popMenu").attr("visibility", "visible");
            }
    ...
    this.GetUrlParam = function (name) {
        ...
        //获取图元对象
        this.getShape = function (uid) {
            var shape = $("#shape" + uid);
            return shape;
        }
        //设置某个值
        this.setShapeValue = function (uid, value, time) {

            SCADA.UpdateFlowValue(value, time, 1, "iovalue", uid);
        }
        //显示图元
        this.showShape = function (uid) {
            var shape = $("#shape" + uid);
            if (shape != undefined && shape != null) {
                shape.attr("style", "display:block;");
                shape.attr("visibility", "visible");
            }

        }
        //隐藏图元
        this.hideShape = function (uid) {
            var shape = $("#shape" + uid);
            if (shape != undefined && shape != null) {
        ...
        //获取IO值
        this.getIoValue = function (server, communicate,device,para) {

            IoParas.push({
                ShapeID: "",
                ServerID: server,
                CommunicateID: communicate,
                DeviceID: device,
                ParaID: para,
                DataType: "value",
                Format:2,
            ...
            var resultvalue = { value: -9999, qualityStamp: 'BAD', dateTime: "1997-01-01 00:00:00" };
            //获取实时数据
            $.ajaxSetup({ async: false });
            $.post("/Scada/ScadaFlow/GetReadData", { ioParas }, function (result) {
                if (result.count > 0) {
                    resultvalue = { value: result.data[0].value, qualityStamp: result.data[0].qualityStamp, dateTime: result.data[0].dateTime };
                }
            });
        ...
        //写入值
        this.setIoValue = function (inputIo, timeout, value) {
            ...
            SCADA.sendIoCommand(cmd);

        }
        this.sendIoCommand = function (cmd) {
            $.ajaxSetup({ async: false });
            $.post(SCADA.CommandUrl, cmd, function (result) {
                if (result.result) {
    ...
    this.RealDataTimer = function (func) {
        func();
        $('#tableId').stopTime("RealTimer");
        $('#tableId').everyTime(refreshtime,"RealTimer", func);
   
    };
    this.RealDataCustumTimer = function (func, time) {
        func();
        $('body').stopTime("CustumTimer");
        $('body').everyTime(time + 's',"CustumTimer", func);
    };
    ...
    this.UpdateFlowValue = function (iovalue, time, status, datatype, uid) {
        ...
        try {
            ...
            if (shapeg == undefined)
                return;
            try {
                var tempminvalue = shapeg.data("minvalue");
                var tempmaxvalue = shapeg.data("maxvalue");
                if (tempminvalue != undefined && tempminvalue != null && tempmaxvalue != undefined && tempmaxvalue != null) {
                    if (value < parseFloat(tempminvalue) || value > parseFloat(tempmaxvalue)) {
                        return;
                    }
                }
            }
            ...
            //判读数据是否同一时间，如果时间相同则丢弃不绘制
            var predate = shapeg.data("predate");
            if (predate != undefined && predate != null && predate != "") {
            ...
            switch (shapeName) {
                ...
                case "SVG_CoolFanShape"://风机
                    {
                        ...
                        var fv = analysisOpeartor(falseop, falsevalue, value);
                        if (tv == true) {
                            animateTransform[0].setAttribute("dur", speed+"ms");

                        }
                        else if (fv == true) {


                            animateTransform[0].setAttribute("dur", "999999999s");
                        }
                    }
                    break;
                ...
                case "SVG_IOLabelShape"://IO显示器
                    {
                        var valuestyle = $("#shape" + uid).data("valuestyle");
                        var minvalue = parseFloat( $("#shape" + uid).data("minvalue"));
                        var maxvalue = parseFloat($("#shape" + uid).data("maxvalue"));
                        ...
                        if (datatype == "DateTime") {
                            ...
                            if (valuestyle == "PercentValue") {
                                var realv = parseFloat( value) / (maxvalue - minvalue) * 100;
                                var v = number_format(realv, ioformat, '.', ',', 'round');
                                if (text != undefined) {
                                    text.html(v+"%");
                                    text.text(v + "%");
                                ...
                                var v = number_format(parseFloat(value), ioformat, '.', ',', 'round');
                                if (text != undefined) {
                                    text.html(v);
                                    text.text(v);
                        ...
                        if (rectstr != undefined) {
                            ...
                            if (buttonmode == "Text") {
                                ...
                                if (tv == true) {
                                    ...
                                    path[2].setAttribute("fill", truecolor);
                                    ...
                                    path[0].setAttribute("fill", falsecolor);
                                    path[1].setAttribute("fill", falsecolor);
                                }
                ...
                case "SVG_BlowerShape"://绘制风机
                    {
                        ...
                        var fv = analysisOpeartor(falseop, falsevalue, value);
                        if (tv == true) {
                            animateTransform[0].setAttribute("dur", "3000ms");

                        }
                        else if (fv == true) {


                            animateTransform[0].setAttribute("dur", "999999999999s");
                        }
                    }
                    break;
                ...
                case "SVG_Chart"://SVG实时曲线图处理
                    {
                        ...
                        var valuearray = vstr.split(",");
                        var daearray = datestr.split(",");
                        //如果超过了指定了的最大点数，则删除之前的点 100,12,21,25,38,90

                       

                        if (valuearray.length > maxpoint) {

                            valuearray.splice(0, valuearray.length - maxpoint);
                            daearray.splice(0, daearray.length - maxpoint);

                        }

                      
                        shapeg.data("value", valuearray.join(","));
                        shapeg.data("date", daearray.join(","));
                        for (var i = 0; i < valuearray.length; i++) {
                            var nv = valuearray[i];
                            if (nv == null || nv == "")
                                continue;
                            var nv = parseFloat(valuearray[i]);
                            if (i == 0) {
                                minvalue = nv;
                                maxvalue = nv;
                        ...
                        for (var i = 0; i < daearray.length; i++) {
                            var nd = new Date(daearray[i].replace(/\-/g, "/"));
                            if (i == 0) {
                                maxdate = nd;
                                mindate = nd;
                            }
                        ...
                        for (var i = 0; i < valuearray.length - 1; i++) {
                            try {
                                ...
                                var nv = valuearray[i];
                                var pvalue1 = parseFloat(valuearray[i]);
                                var pdate1 = new Date(daearray[i]);
                                var pvalue2 = parseFloat(valuearray[i + 1]);
                                var pdate2 = new Date(daearray[i + 1]);
                                ...
                                if (value != "-9999" && value != "") {
                                    ...
                                    try {
                                        ...
                                        //设置内容
                                        serielabels[i].textContent = number_format(nv, ioformat, null, null, null);
                        ...
                        if (parentGroup != undefined) {
                            ...
                            if (xaxislabels != undefined && xaxislabels.length > 0) {
                                ...
                                for (var i = 0; i < xaxislabels.length; i++) {
                                    ...
                                    xaxislabels[i].textContent = currntDate.Format(datestr)+" "+currntDate.Format(timestr);
                                    ...
                                    if (colparentShape != undefined && colparentShape != null) {
                                        if ($("#" + colparentShape[0].id).data("value") != "-9999") {
                                            ...
                                            if (colValue != undefined && colValue != "-9999") {
                                                var h = (colValue - minvalue) * serierect.Height / Math.abs(maxvalue - minvalue);
                ...
                case "SVG_RankingBarChartItemShape"://轮播排名图
                    ...
                    break;
                case "SVG_ArcRulerBaseIOShape"://仪表基础组件
                    {
                        ...
                        //表盘跨度角度
                        var meterDegrees = parseFloat(shapeg.data("meterdegrees"));
                        var minValue = parseFloat(shapeg.data("minvalue"));//表盘最小值
                        var maxValue = parseFloat(shapeg.data("maxvalue"));//表盘最大值
                        var valueType = shapeg.data("valuetype");//要显示的值类型
                        var fltStartAngle = -90 - (meterDegrees) / 2 + 360;
                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        var m_rectWorking = { x: rect.x + 10, y: rect.y + 10, width: rect.width - 20, height: rect.height - 20, top: rect.y + 10, bottom: rect.y + 10 + rect.height - 20, left: rect.x + 10, right: rect.x + 10 + rect.width - 20 };//值域工作区
                        //计算指针的坐标
                        var fltValueAngle = (fltStartAngle + ((value - minValue) / (maxValue - minValue)) * meterDegrees - 180) % 360;
                        var intValueY1 = (m_rectWorking.top + m_rectWorking.width / 2 - ((m_rectWorking.width / 2 - 30) * Math.sin(3.1415926 * (fltValueAngle / 180.00))));
                        var intValueX1 = (m_rectWorking.left + (m_rectWorking.width / 2 - ((m_rectWorking.width / 2 - 30) * Math.cos(3.1415926 * (fltValueAngle / 180.00)))));
                    ...
                    break;
                case "SVG_ArcFaceProcessIOShape":
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                        ...
                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        var processarc = $("#shape" + uid + ">.ioupdate");
                        if (processarc != undefined && processarc != null) {

                            var bls = (value - minValue) / (maxValue - minValue);
                            var fltStartAngle = -90 - (meterDegrees) / 2 + 360;
                            var processAngle = bls * meterDegrees;

                            var pathd = GetArcPath(rect, fltStartAngle, processAngle);
                            pathd += " L" + parseFloat(rect.x + rect.width / 2) + "," + parseFloat(rect.y + rect.height / 2) + "Z";
                            processarc.attr("d", pathd);
                        }
                        var textlabel = $("#shape" + uid + ">text[data-filter='labelvalue']");
                        if (textlabel != undefined && textlabel != null) {
                            ...
                            textlabel.html(strFormat);
                            textlabel.text(strFormat);
                        }
                    }
                    break;
                case "SVG_ArcProcessIOShape":
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                ...
                case "SVG_PointerRulerShape":
                    {
                        ...
                        var maxValue = parseFloat(shapeg.data("maxvalue"));//表盘最大值
                        var valueType = shapeg.data("valuetype");//要显示的值类型
                        var   pointersize = parseFloat(shapeg.data("pointersize"));
                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        ...
                        if (processarc != undefined && processarc != null) {
                            ...
                            var fltValueAngle = (fltStartAngle + ((value - minValue) / (maxValue - minValue)) * meterDegrees) % 360;
                            var intValueY1 = rect.cy + rect.height / 2 * Math.sin(Math.PI * (fltValueAngle / 180.00));
                            var intValueX1 = rect.cx + rect.width / 2 * Math.cos(Math.PI * (fltValueAngle / 180.00));

                            var p1 = { x: intValueX1, y: intValueY1 };

                            var intValueY2 = rect.cy + pointersize * Math.sin(Math.PI * ((fltValueAngle - 40) / 180.00));
                            var intValueX2 = rect.cx + pointersize * Math.cos(Math.PI * ((fltValueAngle - 40) / 180.00));
                            var p2 = { x: intValueX2, y: intValueY2 };
                            var intValueY3 = rect.cy + pointersize * Math.sin(Math.PI * ((fltValueAngle + 40) / 180.00));
                            var intValueX3 = rect.cx + pointersize * Math.cos(Math.PI * ((fltValueAngle + 40) / 180.00));
                            var p3 = { x: intValueX3, y: intValueY3 };


                            var intValueY4 = rect.cy + (rect.height / 8) * Math.sin(Math.PI * ((fltValueAngle + 180) / 180.00));
                            var intValueX4 = rect.cx + (rect.width / 8) * Math.cos(Math.PI * ((fltValueAngle + 180) / 180.00));
                            var p4 = { x: intValueX4, y: intValueY4 };

                            var pathd = "M" + p1.x + "," + p1.y + " L" + p2.x + "," + p2.y + " L" + p4.x + "," + p4.y + " L" + p3.x + "," + p3.y+"Z";
                            processarc.attr("d", pathd);
                        }
                        //data-filter='labelvalue'
                        var textlabel = $("#shape" + uid + ">text[data-filter='labelvalue']");
                    ...
                    break;
                case "SVG_ColorChangedRulerBaseShape"://颜色变化组件
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                            return;
                        //表盘跨度角度
                        var fillShape = $("#shape" + uid + ">path[data-filter='colorchangedfill']");
                        var filltype =shapeg.data("filltype");//填充类型
                        var linearmode = shapeg.data("linearmode");//渐变模式
                        var normalfill =shapeg.data("normalfill");//正常值填充色
                        var fillcolor = normalfill;
                        var expressions =shapeg.data("expressions");//条件表达式
                        var num = expressions.split(",").length;
                        var expressionObjects = [];
                        for (var i = 0; i < num; i++) {
                            var expressionString = expressions.split(",")[i];
                            expressionObjects.push({
                                OpSymbol: expressionString.split(":")[0],
                                Value: parseFloat( expressionString.split(":")[1]),
                                ExpressionColor: expressionString.split(":")[2]


                            });
                        }
                        for (var i = 0; i < expressionObjects.length; i++) {

                            switch (expressionObjects[i].OpSymbol) {
                                case "<":
                                    if (value < expressionObjects[i].Value) {
                                        fillcolor = expressionObjects[i].ExpressionColor;
                                    }
                                    break;
                                case "<=":
                                    {
                        ...
                        switch (filltype) {
                            ...
                            case "颜色填充":
                                {
                                    fillShape.attr("fill", fillcolor);
                                }
                                break;
                            case "渐进填充":
                                {
                                    var fillShapes = $("#shape" + uid + ">defs>linearGradient>stop");
                                    if (fillShapes.length > 0) {
                                        switch (linearmode) {
                                            case "水平渐变":
                                                {
                                                    var linFill = $("#" + fillShapes[0].id);
                                                    var opacity = linFill.css("stop-opacity");
                                                    linFill.css("stop-color", fillcolor);
                                                    linFill.css("stop-opacity", opacity);
                                                }
                                                break;
                                            case "垂直渐变":
                                                {
                                            ...
                                            case "垂直居中渐变":
                                                {
                                                    ...
                                                    var linFill2 = $("#" + fillShapes[2].id);
                                                    opacity = linFill2.css("stop-opacity");
                                                    linFill2.css("stop-color", fillcolor);
                                                    linFill2.css("stop-opacity", opacity);
                                                }
                                                break;
                                            case "水平居中渐变":
                                                {
                            ...
                            case "不填充":
                                {
                                    fillShape.attr("fill", "none");
                                }
                                break;
                        }
                        var textShape = $("#shape" + uid + ">text[data-filter='Label']");
                        if (textShape != undefined && textShape != null) {
                            var strFormat = number_format(parseFloat(value), ioformat, '.', ',', 'round');
                            textShape.html(strFormat);
                            textShape.text(strFormat);
                        }
                    }
                    break;
                case "SVG_FaceIOShape"://面变化组件5
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                            return;
                        //表盘跨度角度
                        var fillShape = $("#shape" + uid + ">rect[data-filter='faceioshape']");
                        var valuestyle = shapeg.data("valuestyle");//填充类型
                        var direction = shapeg.data("direction");//填充类型
                        
                        var minValue = parseFloat(shapeg.data("minvalue"));//最小值
                        var maxValue = parseFloat(shapeg.data("maxvalue"));//最大值
                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        var zoom = (value - minValue) / (maxValue - minValue);
                        var disw = zoom * rect.width;//实际的宽度
                        var dish = zoom * rect.height;//实际的高度
                        switch (direction) {
                            case "Left":
                                {
                                    fillShape.attr("x", rect.x + rect.width- disw);
                                    fillShape.attr("width",disw);
                                }
                                break;
                            case "Top":
                                {
                                    fillShape.attr("y", rect.y + rect.height - dish);
                                    fillShape.attr("height", dish);
                                }
                                break;
                            case "Bottom":
                                {
                                    fillShape.attr("y", rect.y);
                                    fillShape.attr("height", dish);
                                }
                                break;
                            case "Right":
                                {
                                    fillShape.attr("x", rect.x);
                                    fillShape.attr("width", disw);
                                }
                                break;
                        ...
                        var textShape = $("#shape" + uid + ">text[data-filter='iovalue']");
                        if (textShape != undefined && textShape != null) {
                            if (valuestyle == "Percent") {
                                var strFormat = number_format(parseFloat(zoom * 100), ioformat, '.', ',', 'round');
                                textShape.html(strFormat + "%");
                                textShape.text(strFormat + "%");
                            }
                            else {
                ...
                case "SVG_LineMoveBaseIOShape"://IO直线移动的
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                        ...
                        var minValue = parseFloat(shapeg.data("minvalue"));// 
                        var maxValue = parseFloat(shapeg.data("maxvalue"));// 
                        var movepointstyle = shapeg.data("movepointstyle");
                        var labeldock = shapeg.data("labeldock");
                        var sourcepoint = shapeg.data("sourcepoint");
                        var endpoint = shapeg.data("endpoint");
                        //设置进度线
                        var processlineShape = $("#shape" + uid + ">line[data-filter='movelineprocess']");
                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        var blxs = parseFloat((value - minValue) / (maxValue - minValue));
                        var sourcedistance = getDistance({ x: parseFloat(sourcepoint.split(",")[0]), y: parseFloat(sourcepoint.split(",")[1]) }, { x: parseFloat(endpoint.split(",")[0]), y: parseFloat(endpoint.split(",")[1]) });
                        var distance = sourcedistance * blxs;
                     
                        var movepoint = getLinePointFormDistance({ x: parseFloat(sourcepoint.split(",")[0]), y: parseFloat(sourcepoint.split(",")[1]) }, { x: parseFloat(endpoint.split(",")[0]), y: parseFloat(endpoint.split(",")[1]) }, distance);
                        if (processlineShape != undefined && processlineShape != null) {
                            processlineShape.attr("x2", movepoint.x);
                            processlineShape.attr("y2", movepoint.y);
                            var pointrect;
                            switch (movepointstyle) {
                                case "Circle":
                                    {
                                        var processpointShape = $("#shape" + uid + ">ellipse[data-filter='movePoint']");
                                        if (processpointShape != undefined && processpointShape != null) {
                                            var pointrectstring = processpointShape.data("rect");
                                             pointrect = { x: parseFloat(pointrectstring.split(',')[0]), y: parseFloat(pointrectstring.split(',')[1]), width: parseFloat(pointrectstring.split(',')[2]), height: parseFloat(pointrectstring.split(',')[3]) };//获取区域对象
                                            processpointShape.attr("cx", movepoint.x);
                                            processpointShape.attr("cy", movepoint.y);
                                        }
                                    }
                                    break;
                                case "Diamond":
                                    {
                                        var processpointShape = $("#shape" + uid + ">path[data-filter='movePoint']");
                                        if (processpointShape != undefined && processpointShape != null) {
                                            var pointrectstring = processpointShape.data("rect");
                                             pointrect = { x: parseFloat(pointrectstring.split(',')[0]), y: parseFloat(pointrectstring.split(',')[1]), width: parseFloat(pointrectstring.split(',')[2]), height: parseFloat(pointrectstring.split(',')[3]) };//获取区域对象

                                            var dpath = getDiamondPath({ X: movepoint.x, Y: movepoint.y  }, pointrect.width, pointrect.height);
                                            processpointShape.attr("d", dpath);
                                        }
                                    }
                                    break;
                                case "Rect":
                                    {
                                        var processpointShape = $("#shape" + uid + ">rect[data-filter='movePoint']");
                                        if (processpointShape != undefined && processpointShape != null) {
                                            var pointrectstring = processpointShape.data("rect");
                                             pointrect = { x: parseFloat(pointrectstring.split(',')[0]), y: parseFloat(pointrectstring.split(',')[1]), width: parseFloat(pointrectstring.split(',')[2]), height: parseFloat(pointrectstring.split(',')[3]) };//获取区域对象

                                            processpointShape.attr("x", movepoint.x - pointrect.width / 2);
                                            processpointShape.attr("y", movepoint.y - pointrect.height/2);
                                        }
                                    }
                                    break;
                                case "Icon":
                                    {
                                        var processpointShape = $("#shape" + uid + ">image[data-filter='movePoint']");
                                        if (processpointShape != undefined && processpointShape != null) {
                                            var pointrectstring = processpointShape.data("rect");
                                             pointrect = { x: parseFloat(pointrectstring.split(',')[0]), y: parseFloat(pointrectstring.split(',')[1]), width: parseFloat(pointrectstring.split(',')[2]), height: parseFloat(pointrectstring.split(',')[3]) };//获取区域对象
                                            processpointShape.attr("x", movepoint.x - pointrect.width / 2);
                            ...
                            if (textShape != undefined && textShape != null) {
                                ...
                                var textsize = { width: parseFloat(textShape.data("textsize").split(",")[0]), height: parseFloat(textShape.data("textsize").split(",")[1]) };
                                var tx;
                                var ty;
                                switch (labeldock) {
                                    case "Top":
                                        {
                                            tx = movepoint.x - textsize.width / 2-5;
                                            ty = movepoint.y - pointrect.height/2 - textsize.height - 5;
                                        }
                                        break;
                                    case "Left":
                                        {
                                            //mMovePoint.ItemRect.X - ts.Width - 5,
                                            //mMovePoint.ItemRect.Y + mMovePoint.ItemRect.Height / 2 - ts.Height / 2
                                            tx = movepoint.x - pointrect.width / 2- textsize.width -5;
                                            ty = movepoint.y + textsize.height - textsize.height / 2;
                                        }
                                        break;
                                    case "Right":
                                        {

                                            tx = movepoint.x + pointrect.width / 2 + 5;
                                            ty = movepoint.y + textsize.height - textsize.height / 2;
                                        }
                                        break;
                                    case "Bottom":
                                        {
                                            tx = movepoint.x - textsize.width /2-5 ;
                                            ty = movepoint.y + pointrect.height / 2 +5;
                                        }
                                        break;
                                }
                                textShape.attr("x", tx);
                ...
                case "SVG_PathLineMoveBaseIOShape"://IO指定路径移动的
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                        ...
                        var processlineShape = $("#shape" + uid + ">path[data-filter='moveProcessLine']");
                        var marks = $("#shape" + uid + ">path[data-filter='markPoint']");
                        if (marks != undefined && marks != null) {
                            var points = [];
                            for (var i = 0; i < marks.length; i++) {
                                var mark = $("#" + marks[i].id);
                                var localpoint = mark.data("localpoint");
                                var markvalue = parseFloat(mark.data("value"));
                                points.push({ x: parseFloat(localpoint.split(",")[0]), y: parseFloat(localpoint.split(",")[1]), value: markvalue });
                            }
                            if (points.length > 0) {
                                var movepoint;
                                var index = -1;
                                for (var i = 1; i < points.length; i++) {
                                    if (value > points[i - 1].value && value <= points[i].value) {
                                        var distance = getDistance({ x: points[i - 1].x, y: points[i - 1].y }, { x: points[i].x, y: points[i].y });
                                        var blxs = parseFloat((value - points[i - 1].value) / (points[i].value - points[i - 1].value));
                                        movepoint = getLinePointFormDistance({ x: points[i - 1].x, y: points[i - 1].y }, { x: points[i].x, y: points[i].y }, blxs * distance);
                                        index = i;
                                        break;
                                    }
                                ...
                                if (index > -1) {
                                    ...
                                    var processPath = "M" + points[0].x + "," + points[0].y;
                                    for (var i = 0; i < index; i++) {

                                        processPath += " L" + points[i].x + "," + points[i].y;
                                    }
                                    processPath += " L" + movepoint.x + "," + movepoint.y;
                                    processlineShape.attr("d", processPath);
                                    ...
                                    var textShape = $("#shape" + uid + ">text[data-filter='movetext']");
                                    if (textShape != undefined && textShape != null) {
                ...
                case "SVG_RulerBaseIOShape"://IO标尺类的
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                            return;
                        //
                        var direction = shapeg.data("direction");//坐标轴方向
                        var scaledirection = shapeg.data("scaledirection");//刻度轴方向
                        var pointorradious = shapeg.data("pointorradious");//指针大小
                        var minValue = parseFloat(shapeg.data("minvalue"));//最小值
                        var maxValue = parseFloat(shapeg.data("maxvalue"));//最大值
                        var majorintervalsize = shapeg.data("majorintervalsize");//最大值
                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        var blxs = parseFloat((maxValue - value) / (maxValue - minValue));
                        switch (direction) {
                            case "Horizontal":
                                {
                                    blxs = parseFloat((value - minValue) / (maxValue - minValue));
                                }
                                break;
                            case "Vertical":
                                {
                                    blxs = parseFloat((maxValue - value) / (maxValue - minValue));
                                }
                                break;
                        }
                        ...
                        //获取指针对象
                        var pointorShape = $("#shape" + uid + ">path[data-filter='pointorfill']");
                        var x = rect.x + rect.width * blxs;
                        var y = rect.y+rect.height- rect.height * blxs;
                        ...
                        if (pointorShape != undefined && pointorShape != null) {

                            var pointorpath = getRulerBasePointorPath(direction, scaledirection, rect, majorintervalsize, pointorradious, x, y);
                            pointorShape.attr("d", pointorpath);
                        }
                        var textShape = $("#shape" + uid + ">text[data-filter='Label']");
                        if (textShape != undefined && textShape != null) {
                            var textsize = { width: parseFloat(textShape.data("textsize").split(",")[0]), height: parseFloat(textShape.data("textsize").split(",")[1]) };
                            var pointorlabelrect = getRulerBasePointorLabelRect(direction, scaledirection, rect, majorintervalsize, pointorradious, x, y, textsize);
                            textShape.attr("x", pointorlabelrect.x);
                            textShape.attr("y", pointorlabelrect.y);
                            var strFormat = number_format(parseFloat(value), ioformat, '.', ',', 'round');
                            textShape.html(strFormat);
                            textShape.text(strFormat);
                        }
                ...
                case "SVG_SingleSerieIOShape"://IO简易取消组件
                    {
                        var quality = "good";
                        if (value.length <= 0)
                            return;
                        ...
                        var minValue = parseFloat(shapeg.data("minvalue"));//最小值
                        var maxValue = parseFloat(shapeg.data("maxvalue"));//最大值
                        var seriemovestyle = shapeg.data("seriemovestyle");
                        var scaleinternal = shapeg.data("scaleinternal");
                        var maxnumber = shapeg.data("maxnumber");//设计的最大显示数据点
                        var pointstr = shapeg.data("points");//存储的数据点
                        var serielinestyle = shapeg.data("serielinestyle");//曲线样式

                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        var blxs = parseFloat((maxValue - value) / (maxValue - minValue));
                        var disw = rect.width / maxnumber;

                        var realnum = pointstr.split(";").length;

                        var points = [];
                        if (pointstr != "") {
                            for (var i = 0; i < pointstr.split(";").length; i++) {

                                points.push({ value: parseFloat(pointstr.split(";")[i].split(",")[0]), quality: pointstr.split(";")[i].split(",")[1] });
                            }
                        }

                        while (points.length >= maxnumber) {
                            points.shift();
                        }

                        
                        points.push({ value: value, quality: quality });
                        pointstr = "";
                        for (var i = 0; i < points.length; i++) {
                            points[i].quality = "good";
                            if (i % 12 == 0) {
                                points[i].quality = "bad";
                            }
                            if (i == 0)
                                pointstr = points[i].value + "," + points[i].quality;
                            else
                                pointstr += ";" + points[i].value + "," + points[i].quality;
                        }
                        shapeg.data("points", pointstr);
                        var serieShape = $("#shape" + uid + ">path[data-filter='serie']");
                        if (serieShape != undefined && serieShape != null) {
                            ...
                            for (var i = 0; i < points.length; i++) {
                                if (points[i].quality == "good") {

                                    var y = rect.y + rect.height - points[i].value / (maxValue - minValue) * rect.height;
                                    var x = rect.x + i * disw;
                                    if (seriemovestyle == "LeftToRight") {
                                        x = rect.x + rect.width - i * disw;
                                    }
                                    if (pathD == "") {
                                        if (serielinestyle == "Area") {
                                            pathD = " M" + x + "," + parseFloat(rect.y + rect.height) + " L" + x + "," + y;;
                                        }
                                        else {
                                            pathD = " M" + x + "," + y;
                                        }
                                ...
                                else if (points[i].quality == "bad") {
                                    if (pathD != "") {
                                        if (serielinestyle == "Area") {
                                            pathD += " L" + prex + "," + parseFloat(rect.y + rect.height) + " Z";
                                        }
                                        pathds.push(pathD);
                                        pathD = "";
                                    }
                            ...
                            serieShape.attr("d", pathds.join(" "));
                        }

                        //写入刻度值
                        var textShapes = $("#shape" + uid + ">text[data-filter='iolabel']");
                        if (textShapes != undefined && textShapes != null) {
                            for (var e = 0; e < textShapes.length; e++)
                            {
                                $("#" + textShapes[e].id).html("");
                                $("#" + textShapes[e].id).text("");
                                var index = $("#" + textShapes[e].id).data("index");
                                if (index < points.length) {
                                    if (points[index].quality == "good") {
                                        var lx;
                                        var ly;
                                        if (seriemovestyle == "LeftToRight") {
                                            lx = rect.x + rect.width - index * disw;
                                            ly = rect.y + rect.height - points[index].value / (maxValue - minValue) * rect.height;
                                        }
                                        else {
                                            lx = rect.x + index * disw;
                                            ly = rect.y + rect.height - points[index].value / (maxValue - minValue) * rect.height;
                                        }
                                        var strFormat = number_format(parseFloat(points[index].value), ioformat, '.', ',', 'round');
                                        $("#" + textShapes[e].id).attr("x",lx);
                                        $("#" + textShapes[e].id).attr("y", ly);
                                        $("#" + textShapes[e].id).html(strFormat);
                                        $("#" + textShapes[e].id).text(strFormat);
                                    }
                ...
                case "SVG_SizedChangedRulerBaseShape"://IO元件大小变化
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                        ...
                        var sizechangedstyle = shapeg.data("sizechangedstyle");//圆角大小
                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        var zoom = (value - minValue) / (maxValue - minValue);
                        var disw = (value - minValue) / (maxValue - minValue) * rect.width;
                        var dish = (value - minValue) / (maxValue - minValue) * rect.height;
                        //transform='translate(295 115)'著作权归作者所有。
                    
                        var fillShape = $("#shape" + uid + ">path[data-filter='sizechangedfill']");
                        fillShape.attr("transform", " translate(" + (rect.x + rect.width / 2) + " " + (rect.y + rect.height / 2) + ")  scale(" + zoom + ") translate(-" + (rect.x + rect.width / 2) + " -" + (rect.y + rect.height / 2) + ")");
                    ...
                    break;
                case "SVG_MoveChangedRulerBaseShape"://IO元件偏移
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                        ...
                        var maxValue = parseFloat(shapeg.data("maxvalue"));//最大值
                        var offiset = {
                            x: parseFloat(shapeg.data("offiset").split(",")[0]), y: parseFloat(shapeg.data("offiset").split(",")[1])
                        };//偏移量
                        var sizechangedstyle = shapeg.data("sizechangedstyle");//圆角大小
                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        var bls = (value - minValue) / (maxValue - minValue);
                        var disw = offiset.x * bls;
                        var dish = offiset.y * bls;
                        //transform='translate(295 115)'著作权归作者所有。

                        var fillShape = $("#shape" + uid + ">path[data-filter='movechangedfill']");
                        fillShape.attr("transform", " translate(" + (rect.x + disw) + " " + (rect.y + dish) + ")");
                        ...
                        if (textShape != undefined && textShape != null) {
                            ...
                            textShape.html(strFormat);
                            textShape.text(strFormat);
                            textShape.attr("transform", " translate(" + (rect.x + disw) + " " + (rect.y + dish) + ")");
                        }
                    }
                    break;
                case "SVG_ColorSwitchBaseShape"://开关颜色变化组件
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                        ...
                        var fillShape = $("#shape" + uid + ">path[data-filter='colorswitchfill']");
                        var filltype = shapeg.data("filltype");//填充类型
                        var linearmode = shapeg.data("linearmode");//渐变模式
                        var normalfill = shapeg.data("normalfill");//正常值填充色
                        var fillcolor = normalfill;
                        var switchdirection = shapeg.data("switchdirection");//条件表达式
                        var truecolor = shapeg.data("truecolor");//true值颜色
                        var falsecolor = shapeg.data("falsecolor");//false值颜色
    
                        var offisetdistance = parseFloat( shapeg.data("offisetdistance"));
                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        rect.cx = rect.x + rect.width / 2;
                        rect.cy = rect.y + rect.height / 2;
                        var trueexpression = shapeg.data("iovaluetrue");
                        ...
                        var tv = analysisOpeartor(trueop, truevalue, value);
                        var fv = analysisOpeartor(falseop, falsevalue, value);
                        if (fillShape != undefined &&fillShape != null)
                        {
                                ...
                                case "RightToLeft":
                                    {
                                        ...
                                        if (tv == true) {
                                            fillShape.attr("transform", "translate(-" + offisetdistance + ",0)");
                                        }
                                        else if (fv == true) {

                                            fillShape.attr("transform", "");
                                        }
                                    }
                                    break;
                                case "LeftToRight":
                                ...
                                case "TopToBottom":
                                    {
                                        ...
                                        if (tv == true) {
                                            fillShape.attr("transform", "translate(0," + offisetdistance+")");
                                        }
                                        else if (fv == true) {
                    ...
                    break;
                case "SVG_ColorSwitchSelfRotationBaseShape"://开关颜色旋转变化组件
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                        ...
                        var trueangle = parseFloat( shapeg.data("trueangle"));//true值角度
                        var falseangle = parseFloat(shapeg.data("falseangle"));//false值角度
                        ...
                        if (fillShape != undefined && fillShape != null) {
                            if (tv == true) {
                                fillShape.attr("transform", "rotate(" + trueangle + "," + rect.cx + "," + rect.cy+")");
                            }
                            else if (fv == true) {
                                fillShape.attr("transform", "rotate(" + falseangle + "," + rect.cx + "," + rect.cy +")");
                            }
                        }
                        if (tv == true) {
                            fillcolor = truecolor;
                    ...
                    break;
                case "SVG_ColorSwitchCenterRotationBaseShape"://开关颜色旋转变化组件
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                        ...
                        var fillcolor = normalfill;
                        var offiset = {};
                         offiset.x = parseFloat(shapeg.data("offiset").split(',')[0]);
                         offiset.y = parseFloat(shapeg.data("offiset").split(',')[1]);
                        var trueangle = parseFloat(shapeg.data("trueangle"));//true值角度
                        var falseangle = parseFloat(shapeg.data("falseangle"));//false值角度
                        ...
                        if (fillShape != undefined && fillShape != null) {
                            if (tv == true) {
                                fillShape.attr("transform", "rotate(" + trueangle + "," + parseFloat(offiset.x) + "," + parseFloat( offiset.y) + ")");
                            }
                            else if (fv == true) {
                                fillShape.attr("transform", "rotate(" + falseangle + "," + parseFloat(offiset.x) + "," + parseFloat(offiset.y) + ")");
                            }
                        }
                        if (tv == true) {
                            fillcolor = truecolor;
                    ...
                    break;
                case "SVG_SwitchTextShape":
                    {
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                            return;
                        var textShape = $("#shape" + uid + ">text[data-filter='infolabel']");
                        var truecolor = shapeg.data("truecolor");//true值颜色
                        var falsecolor = shapeg.data("errorcolor");//false值颜色
                        var truetext = shapeg.data("normaltext");//true值文本
                        var falsetext = shapeg.data("errortext");//false值文本
                        var trueexpression = shapeg.data("iovaluetrue");
                        var trueop = trueexpression.split(":")[0];
                        var truevalue = parseFloat(trueexpression.split(":")[1]);
                        var falseexpression = shapeg.data("iovaluefalse");
                        ...
                        if (textShape != undefined ) {
                            ...
                            if (tv == true) {
                                textShape.attr("fill", truecolor);
                                textShape.html(truetext);
                                textShape.text(truetext);
                            }
                            else if (fv == true) {

                                textShape.attr("fill", falsecolor);
                                textShape.html(falsetext);
                                textShape.text(falsetext);
                            }
                        }
                    }
                    break;
                case "SVG_CustumElementShape"://自定义组件的动态Io实现
                    {
                        //element
                        if (value.length <= 0)
                            return;
                        if (value == -9999 || value == "-9999")
                            return;
                        var elementname = shapeg.data("element");
                        var maxvalue = parseFloat( shapeg.data("maxvalue"));
                        var minvalue = parseFloat(shapeg.data("minvalue"));
                        var forecolor = shapeg.data("forecolor");
                        var backcolor = shapeg.data("backcolor");
                        var banding = shapeg.data("banding");
                        var changedstyle = shapeg.data("changedstyle");
                        var linearmode = shapeg.data("linearmode");
                        var filltype = shapeg.data("filltype");
                        var rect = { x: parseFloat(rectstr.split(',')[0]), y: parseFloat(rectstr.split(',')[1]), width: parseFloat(rectstr.split(',')[2]), height: parseFloat(rectstr.split(',')[3]) };//获取区域对象
                        rect.cx = rect.x + rect.width / 2;
                        rect.cy = rect.y + rect.height / 2;
                        ...
                        if (banding != "不绑定IO") {
                            var faceioshape = $("#shape" + uid + ">path[data-filter='faceioshape']");
                            var sizedirection = shapeg.data("sizedirection");
                            var iopara = {};
                            if (banding == "IO开关变量") {
                                var color = shapeg.data("color");
                                var opacity = shapeg.data("opacity");
                                var offisetx = shapeg.data("offisetx");
                                var offisety = shapeg.data("offisety");
                                var size = shapeg.data("size");
                                var angle = shapeg.data("angle");
                                iopara.falsecolor = color.split(",")[0];
                                iopara.truecolor = color.split(",")[1];
                                iopara.falseopacity = parseFloat( opacity.split(",")[0]);
                                iopara.trueopacity = parseFloat(opacity.split(",")[1]);
                                iopara.falseoffisetx = parseFloat(offisetx.split(",")[0]);
                                iopara.trueoffisetx = parseFloat(offisetx.split(",")[1]);
                                iopara.falseoffisety = parseFloat(offisety.split(",")[0]);
                                iopara.trueoffisety = parseFloat(offisety.split(",")[1]);
                                iopara.falsesize = parseFloat(size.split(",")[0]);
                                iopara.truesize = parseFloat(size.split(",")[1]);
                           
                                iopara.falseangle = parseFloat(angle.split(",")[0]);
                                iopara.trueangle = parseFloat(angle.split(",")[1]);
                                if (faceioshape != null && faceioshape != undefined) {
                                    var trueexpression = shapeg.data("iovaluetrue");
                                    var trueop = trueexpression.split(":")[0];
                                    var truevalue = parseFloat(trueexpression.split(":")[1]);
                                    var falseexpression = shapeg.data("iovaluefalse");
                                        ...
                                        case "大小变化":
                                            {
                                                ...
                                                switch (sizedirection) {
                                                    case "向左":
                                                        {
                                                            iopara.falsesize = iopara.falsesize/100;
                                                            iopara.truesize = iopara.truesize/100;
                                                            if (tv == true) {
                                                                shapeg.attr("transform", "scale(" + iopara.truesize + ",1) translate(-" + (rect.width * (1 - iopara.truesize)) + ",0)");
                                                            }
                                                            else if (fv == true) {

                                                                shapeg.attr("transform", "scale(" + iopara.falsesize + ",1) translate(-" + (rect.width * (1 - iopara.falsesize)) + ",0)");
                                                            }
                                                    ...
                                                    case "向下":
                                                        {
                                                            ...
                                                            iopara.truesize = iopara.truesize / 100;
                                                            if (tv == true) {
                                                                shapeg.attr("transform", "scale(1," + iopara.truesize + ") translate(0,-" + (rect.height * (1 -iopara.truesize)) + ")");
                                                            }
                                                            else if (fv == true) {

                                                                shapeg.attr("transform", "scale(1," + iopara.falsesize + ") translate(0,-" + (rect.height * (1 - iopara.falsesize)) + ")");
                                                            }
                                                        }
                                                        break;
                                                    case "向上":
                                                    ...
                                                    case "由中向外":
                                                        {
                                                            ...
                                                            if (tv == true) {
                                                                shapeg.attr("transform", "scale(" + iopara.truesize + "," + iopara.truesize+")");
                                                            }
                                                            else if (fv == true) {

                                                                shapeg.attr("transform", "scale(" + iopara.falsesize + "," + iopara.falsesize+")");
                                                    ...
                                                    case "向左":
                                                        {
                                                            ...
                                                            iopara.truesize = iopara.truesize / 100;
                                                            if (tv == true) {
                                                                shapeg.attr("transform", "   translate(-" + rect.width / 2 + ",0)  scale(" + iopara.truesize + ",1)  translate(" + (rect.x * (1 - iopara.truesize) + rect.width * iopara.truesize / 2) + ",0)");
                                                            }
                                                            else if (fv == true) {

                                                                shapeg.attr("transform", "   translate(-" + rect.width / 2 + ",0)  scale(" + iopara.falsesize + ",1)  translate(" + (rect.x * (1 - iopara.falsesize) + rect.width * iopara.falsesize / 2) + ",0)");
                                                            }
                                                    ...
                                                    case "向下":
                                                        {
                                                            ...
                                                            iopara.truesize = iopara.truesize / 100;
                                                            if (tv == true) {
                                                                shapeg.attr("transform", "   translate(0,-" + rect.height / 2 + ")  scale(1," + iopara.truesize + ")  translate(0," + (rect.y * (1 - iopara.truesize) + rect.height * iopara.truesize  / 2) + ")");
 
                                                            }
                                                            else if (fv == true) {

                                                                shapeg.attr("transform", "   translate(0,-" + rect.height / 2 + ")  scale(1," + iopara.falsesize + ")  translate(0," + (rect.y * (1 - iopara.falsesize) + rect.height * iopara.falsesize / 2) + ")");
                                                            }
                                                    ...
                                                    case "由中向外":
                                                        {
                                                            ...
                                                            if (tv == true) {
                                                                shapeg.attr("transform", "scale(" + iopara.truesize + ")");
                                                            }
                                                            else if (fv == true) {

                                                                shapeg.attr("transform", "scale(" + iopara.falsesize + ")");
                                                    ...
                                                    case "横向居中":
                                                        {
                                                            ...
                                                            if (tv == true) {
                                                                shapeg.attr("transform", "scale(" + iopara.truesize + ",1)");
                                                            }
                                                            else if (fv == true) {

                                                                shapeg.attr("transform", "scale(" + iopara.falsesize + ",1)");
                                                    ...
                                                    case "纵向居中":
                                                        {
                                                            ...
                                                            if (tv == true) {
                                                                shapeg.attr("transform", "scale(1," + iopara.truesize + ")");
                                                            }
                                                            else if (fv == true) {

                                                                shapeg.attr("transform", "scale(1," + iopara.falsesize + ")");
                                        ...
                                        case "颜色变化":
                                            {
                                                if (filltype == "不填充") {
                                                    if (tv == true) {
                                                        faceioshape.attr("fill", iopara.truecolor);
                                                    }
                                                    else if (fv == true) {

                                                        faceioshape.attr("fill", iopara.falsecolor);
                                                    }
                                                }
                                                else if (filltype == "颜色填充") {
                                                    if (tv == true) {
                                                        faceioshape.attr("fill", iopara.truecolor);
                                                    }
                                                    else if (fv == true) {
                                                ...
                                                else if (filltype == "渐进填充") {
                                                 
                                                   
                                                    switch (linearmode) {
                                                        case "放射性渐变":
                                                            {
                                                                var fillShapes = $("#shape" + uid + ">defs>radialGradient>stop");

                                                                if (tv == true) {

                                                                    var linFill = $("#" + fillShapes[0].id);
                                                                    var opacity = linFill.css("stop-opacity");
                                                                    linFill.css("stop-color", iopara.truecolor);
                                                                    linFill.css("stop-opacity", opacity);
                                                                    ...
                                                                    var linFill = $("#" + fillShapes[0].id);
                                                                    var opacity = linFill.css("stop-opacity");
                                                                    linFill.css("stop-color", iopara.falsecolor);
                                                                    linFill.css("stop-opacity", opacity);
                                                            ...
                                                            case "水平渐变":
                                                                {
                                                                    ...
                                                                    if (tv == true) {
                                                                        ...
                                                                        var linFill = $("#" + fillShapes[0].id);
                                                                        var opacity = linFill.css("stop-opacity");
                                                                        linFill.css("stop-color", truecolor);
                                                                        linFill.css("stop-opacity", opacity);
                                                                        ...
                                                                        var linFill = $("#" + fillShapes[0].id);
                                                                        var opacity = linFill.css("stop-opacity");
                                                                        linFill.css("stop-color", falsecolor);
                                                                        linFill.css("stop-opacity", opacity);
                                                            ...
                                                            case "垂直居中渐变":
                                                                {
                                                                    ...
                                                                    if (tv == true) {
                                                                        ...
                                                                        var linFill2 = $("#" + fillShapes[2].id);
                                                                        opacity = linFill2.css("stop-opacity");
                                                                        linFill2.css("stop-color", iopara.truecolor);
                                                                        linFill2.css("stop-opacity", opacity);
                                                                        ...
                                                                        var linFill2 = $("#" + fillShapes[2].id);
                                                                        opacity = linFill2.css("stop-opacity");
                                                                        linFill2.css("stop-color", iopara.falsecolor);
                                                                        linFill2.css("stop-opacity", opacity);
                                                                    }
                                        ...
                                        case "位置移动变化":
                                            {
                                                if (tv == true) {
                                                    ...
                                                    shapeg.attr("transform", " translate(" + (iopara.trueoffisetx) + "," + (iopara.trueoffisety) + ")");
                                                }
                                                else if (fv == true) {

                                                    shapeg.attr("transform", " translate(" + ( iopara.falseoffisetx) + "," + ( iopara.falseoffisety) + ")");
                                                }
                                            }
                                            break;
                                        case "旋转变化":
                                            {
                                                if (tv == true) {
                                                    shapeg.attr("transform", "rotate(" + iopara.trueangle + ")");
                                                
                                                }
                                                else if (fv == true) {
                                                    shapeg.attr("transform", "rotate(" + iopara.falseangle + ")");
                                                }
                                            }
                                            break;
                                    }
                            ...
                            {
                                var faceioshape = $("#shape" + uid + ">rect[data-filter='faceioshape']");
                                var sizedirection = shapeg.data("sizedirection");
                                ...
                                iopara.minopacity = parseFloat(opacity.split(",")[0]);
                                iopara.maxopacity = parseFloat(opacity.split(",")[1]);
                                iopara.minoffisetx = parseFloat(offisetx.split(",")[0]);
                                iopara.maxoffisetx = parseFloat(offisetx.split(",")[1]);
                                iopara.minoffisety = parseFloat(offisety.split(",")[0]);
                                iopara.maxoffisety = parseFloat(offisety.split(",")[1]);
                                iopara.minsize = parseFloat(size.split(",")[0]);
                                iopara.maxsize = parseFloat(size.split(",")[1]);
                                iopara.minangle = parseFloat(angle.split(",")[0]);
                                iopara.maxangle = parseFloat(angle.split(",")[1]);
                                ...
                                if (faceioshape != null && faceioshape != undefined) {
                                    switch (changedstyle) {
                                                    ...
                                                    case "向左":
                                                        {
                                                            ...
                                                            iopara.minsize = iopara.minsize / 100;
                                                            iopara.maxsize = iopara.maxsize / 100;
                                                            var currsize = iopara.minsize + (value - minvalue) / (maxvalue - minvalue) * (iopara.maxsize - iopara.minsize);
                                                           

                                                            shapeg.attr("transform", "   translate(-" + rect.width/2 + ",0)  scale(" + currsize + ",1)  translate(" + (rect.x * (1 - currsize) + rect.width * currsize / 2) + ",0)");
                                                    ...
                                                    case "向下":
                                                        {
                                                            ...
                                                            iopara.maxsize = iopara.maxsize / 100;
                                                            var currsize = iopara.minsize + (value - minvalue) / (maxvalue - minvalue) * (iopara.maxsize - iopara.minsize);
                                                            shapeg.attr("transform", "   translate(0,-" + rect.height / 2 + ")  scale(1," + currsize + ")  translate(0," + (rect.y * (1 - currsize) + rect.height * currsize / 2) + ")");
                                                    ...
                                                    case "由中向外":
                                                        {
                                                            ...
                                                            iopara.maxsize = iopara.maxsize / 100;
                                                            var currsize = iopara.minsize + (value - minvalue) / (maxvalue - minvalue) * (iopara.maxsize - iopara.minsize);
                                                            shapeg.attr("transform", "scale(" + currsize + "," + currsize+")  ");
                                        ...
                                        case "颜色变化":
                                            {
                                                if (filltype == "不填充") {
                                                    ...
                                                    var opacity = iopara.minopacity + (value - minvalue) / (maxvalue - minvalue) * (iopara.maxopacity - iopara.minopacity);
                                                        faceioshape.attr("fill-opacity", opacity);
                                                }
                                                else if (filltype == "渐进填充") {
                                                    var opacityvalue = iopara.minopacity + (value - minvalue) / (maxvalue - minvalue) * (iopara.maxopacity - iopara.minopacity);
                                                            ...
                                                            case "放射性渐变":
                                                                {
                                                                    ...
                                                                    linFill.css("stop-opacity", opacityvalue);
                                                            ...
                                                            case "垂直居中渐变":
                                                                {
                                                                    ...
                                                                    linFill2.css("stop-opacity", opacityvalue);
                                                            ...
                                                            case "无渐变":
                                                                {
                                                                    ...
                                                                    faceioshape.attr("fill-opacity", opacityvalue);
                                                                }
                                                                break;
                                                        }
                                        ...
                                        case "位置移动变化":
                                            {
                                                var offisetx = iopara.minoffisetx + (iopara.maxoffisetx - iopara.minoffisetx)* (value - minvalue) / (maxvalue - minvalue);
                                                var offisety = iopara.minoffisety + (iopara.maxoffisety - iopara.minoffisety) * (value - minvalue) / (maxvalue - minvalue);
                                                shapeg.attr("transform", " translate(" + ( offisetx) + "," + (offisety) + ")");
                                            }
                                            break;
                                        case "旋转变化":
                                            {
                                                var angle = iopara.minangle + (iopara.maxangle - iopara.minangle)* (value - minvalue) / (maxvalue - minvalue);
                                                shapeg.attr("transform", "rotate(" + angle + ")");
                                            }
                                            break;
                                        case "填充变化":
                                            {
        ...
        ///动态修改读取事件信息的图元
        this.UpdateFlowEventValue = function (eventvalue, uid) {
            ...
            try {
                ...
                if (shapeg == undefined)
                    return;
                var shapeName = shapeg.data("shape");
                var event = shapeg.data("ioevent");
                switch (shapeName) {
                    //圆形进度条动态修改,此处重新修改Path参数
                    case "SVG_ScadaEventShape":
                        {
                            var eventcontent = $("#shape" + uid + ">foreignObject>.ioevent");
                            
                    
                            if (eventcontent[0].options.length > 5000) {
                                eventcontent[0].options.length = 0;
                            }
                            for (var i = 0; i < eventvalue.length; i++) {
                                var opt = new Option(eventvalue[i].DateTime+" "+eventvalue[i].Value, eventvalue[i].DateTime);

                                if (eventcontent[0].length > 0)
                                    eventcontent[0].insertBefore(opt, eventcontent[0].options[0]);
                                else
                                    eventcontent[0].add(opt);
                            }
        ...
        //动态修改设备状态信息
        this.UpdateFlowStatusValue = function (statusvalue, time, uid) {
            ...
            try {
                ...
                if (shapeg == undefined)
                    return;
                var shapeName = shapeg.data("shape");
                var status = shapeg.data("iostatus");
                var rectstr = shapeg.data("rect");
                switch (shapeName) {
                    //圆形进度条动态修改,此处重新修改Path参数
                    case "SVG_DeviceStatusShape"://信号状态
                        {
                            ...
                            var tv = parseInt(statusvalue) == 1 ? true : false;
                            if (statustype == "Text") {
                                var text = $("#shape" + uid + ">.iotext");
        ...
        //机器训练模块
        this.UpdateFlowMachineTrainValue = function (train, uid) {
            ...
            try {
                ...
                if (shapeg == undefined)
                    return;
                var shapeName = shapeg.data("shape");
                var machineTrain = shapeg.data("iomachinetrain");
                var rectstr = shapeg.data("rect");
                switch (shapeName) {
                    //圆形进度条动态修改,此处重新修改Path参数
                    case "SVG_MachineTrainSingleWorkShape"://信号状态
                        {
                            var texteditors = $("#shape" + uid + ">text[data-filter]");
                            for (var e = 0; e < texteditors.length; e++) {
                                try {
                                    var text = $("#" + texteditors[e].id);
                                    switch (text.data("filter")) {
                                        case "date":
                                            {
                                                text.html(train.ForecastDate);
                                                text.text(train.ForecastDate);
                                            }
                                            break;
                                        case "prediction":
                                            {
                                                text.html(train.ForecastLabel);
                                                text.text(train.ForecastLabel);
                                            }
                                            break;
                                        case "predictioninput":
                                            {
                                                text.html(train.ForecastInputValues);
                                                text.text(train.ForecastInputValues);
                                            }
                                            break;
                                        case "predictioniopara":
                                            {
                                                text.html(train.ForecastIOParaNames);
                                                text.text(train.ForecastIOParaNames);
                                            }
                                            break;
                                        case "predictionscore":
                                            {
                                                text.html(train.ForecastScore);
                                                text.text(train.ForecastScore);
                                            }
                                            break;
                                        case "taskname":
                                            {
                                                text.html(train.TaskName);
                                                text.text(train.TaskName);
                                            }
                                            break;
                                        case "algorithm":
                                            {
                                                text.html(train.Algorithm);
                                                text.text(train.Algorithm);
                                            }
                                            break;
                                        case "algorithmtype":
                                            {
                                                text.html(train.AlgorithmType);
                                                text.text(train.AlgorithmType);
                                            }
                                            break;
                                        case "communicationname":
                                            {
                                                text.html(train.COMM_NAME);
                                                text.text(train.COMM_NAME);
                                            }
                                            break;
                                        case "servername":
                                            {
                                                text.html(train.SERVER_NAME);
                                                text.text(train.SERVER_NAME);
                                            }
                                            break;
                                        case "devicename":
                                            {
                                                text.html(train.DEVICE_NAME);
                                                text.text(train.DEVICE_NAME);
                                            }
                                      
                                            break;
                                        case "forecastpriod":
                                            {
                                                text.html(train.ForecastPriod+"分钟/次");
                                                text.text(train.ForecastPriod + "分钟/次");
                                            }
                                            break;
                        ...
                        break;
                    case "SVG_MachineTrainMultiWorkShape"://训练日志模式显示
                        {

                            var eventcontent = $("#shape" + uid + ">foreignObject>.iomachinelist");
                            if (eventcontent[0].options.length> 5000) {
                                eventcontent[0].options.length = 0;
                            }
                         
                            var opt = new Option(train.ForecastDate + "  " + train.ForecastLabel + "  " + train.ForecastInputValues + "  " + train.ForecastIOParaNames + "  " + train.TaskName + "  " + train.SERVER_NAME + "  " + train.COMM_NAME + "  " + train.DEVICE_NAME, train.ForecastDate);
        ...
        this.UpdateFlowBatchCommand = function () {

            //BatchCommandTaskShape
            //获取对应的shape 每个shape 有一个g,包含IO参数
            var shapeg = $("#shape" + uid);
            if (shapeg == undefined)
                return;
        ...
        layui.use(["form", "okUtils", "okLayer", 'layer'], function () {
            ...
            function LoadService(dagindex) {
                switch (event_servicetype) {
                    case "链接服务":
                        {
                            window.open(shape.data('event_url'));
                    ...
                    case "下置命令服务":
                        {
                            var writevalue = parseFloat(shape.data('event_writevalue'));
                            ...
                            var event_fixvalue = shape.data('event_fixvalue');

                           
                            
                            var cmd = {
                                writevalue: writevalue,
                                fixvalue: event_fixvalue,
                            ...
                            var readonly = cmd.fixvalue == true ? "readonly" : "";
                            if (event_valuetype == "模拟量") {
                                    ...
                                    content: "<div id='container' class='ok-body'>"
                                               + "       <div class='layui-form-item' >"
                                                  + "     <label class='layui-form-label'>命令下置</label>"
                                                   ...
                                                   + "    <input type='text' id='cmd_write' " + readonly+" name='cmd_write' placeholder='下置值' autocomplete='off' class='layui-input' lay-verify='required' value='" + writevalue + "'>"
                                                   + "      </div>"
                                                  + "     </div>"
                                                 + "   <div class='layui-form-item'>"
                                            + "   <div class='layui-input-block'>"
                                ...
                                var str = "<select  autocomplete='off' class='layui-input' lay-verify='required' id='cmd_write' name='cmd_write' >";

                                if (cmd.writevalue <= 0) {
                                    str += "<option value='0' selected>关[0]</option>";
                                    str += "<option value='1'>开[1]</option>";
                                }

                                else {
                                    str += "<option value='0' >关[0]</option>";
                                    str += "<option value='1' selected>开[1]</option>";
                                }

                               
                                str += "</select>";
                                if (cmd.fixvalue) {
                                    str = "    <input type='text' id='cmd_write' " + readonly+"  name='cmd_write' placeholder='下置值' autocomplete='off' class='layui-input' lay-verify='required' value='" + writevalue + "'>";
                                }
                                //布尔值量直接下置该命令
                                var index = layer.open({
                                    type: 1,
    ...
    this.OnEventServiceFromID = function (eleid) {
                        ...
                        case "下置命令服务":
                            {
                                ...
                                if (event_valuetype == "模拟量") {
                                    ...
                                    var str = "<select  autocomplete='off' id='cmd_write' name='cmd_write'  class='layui-input' lay-verify='required' >";
                                    ...
                                    str += "</select>";
                                    if (cmd.fixvalue) {
                                        str = "    <input type='text' id='cmd_write' name='cmd_write'  " + readonly + "   placeholder='下置值' autocomplete='off' class='layui-input' lay-verify='required' value='" + writevalue + "'>";
                                    }
                                    //布尔值量直接下置该命令
                                    var index = layer.open({
                                        type: 1,
    ...
    this.ReadFlowRealData = function () {
        ...
        var statuselements = document.querySelectorAll('[data-iostatus]');//获取所有要状态图元信息
        var eventelements = document.querySelectorAll('[data-ioevent]');//获取所有要显示事件信息的图元
        var ioelements = document.querySelectorAll('[data-iovalue]');//获取所有要进行实时数据刷新的元素
        var visibleelements = document.querySelectorAll('[data-iovisible]');//获取所有要进行显示与因此的图元
        var changeements = document.querySelectorAll('[data-iochange]');//获取所有要进行颜色变化的图元
        var trainelements = document.querySelectorAll('[data-iomachinetrain]');//获取所有要显示单个机器训练信息的图元
        //由于高德地图数据比较特殊，此处需要单独对地图类的实时显示进行单独设置
        var mapelements = document.querySelectorAll("[data-shape='SVG_MapShape']");
        var trainlistelements = document.querySelectorAll('[data-iomachinetrainlist]');//获取所有要list机器训练显示的图元
        ...
        try {
            for (var i = 0; i < ioelements.length; i++) {
                ...
                if (parastr != undefined && parastr.trim() != "") {
                    IoParas.push({ ShapeID: ioelements[i].id, ServerID: parastr.split(',')[0], CommunicateID: parastr.split(',')[1], DeviceID: parastr.split(',')[2], ParaID: parastr.split(',')[3], DataType: parastr.split(',')[4], Format: parastr.split(',')[5], Unit: parastr.split(',')[6], UpdateCycle: parastr.split(',')[7], IoName: parastr.split(',')[8], Value: '', DateTime: '', Status: 0, QualityStamp: 'BAD' });
                }
            ...
            for (var i = 0; i < visibleelements.length; i++) {
                ...
                if (parastr != undefined && parastr.trim() != "") {
                    IoParas.push({ ShapeID: visibleelements[i].id, ServerID: parastr.split(',')[0], CommunicateID: parastr.split(',')[1], DeviceID: parastr.split(',')[2], ParaID: parastr.split(',')[3], DataType: parastr.split(',')[4], Format: parastr.split(',')[5], Unit: parastr.split(',')[6], UpdateCycle: parastr.split(',')[7], IoName: parastr.split(',')[8], Value: '', DateTime: '', Status: 0, QualityStamp: 'BAD' });
                }
            ...
            for (var i = 0; i < changeements.length; i++) {
                ...
                if (parastr != undefined && parastr.trim() != "") {
                    IoParas.push({ ShapeID: changeements[i].id, ServerID: parastr.split(',')[0], CommunicateID: parastr.split(',')[1], DeviceID: parastr.split(',')[2], ParaID: parastr.split(',')[3], DataType: parastr.split(',')[4], Format: parastr.split(',')[5], Unit: parastr.split(',')[6], UpdateCycle: parastr.split(',')[7], IoName: parastr.split(',')[8], Value: '', DateTime: '', Status: 0, QualityStamp: 'BAD' });
                }
            ...
            for (var i = 0; i < eventelements.length; i++) {
                ...
                var parastr = eventelements[i].getAttribute("data-ioevent");
                if (parastr != undefined && parastr.trim() != "") {
                    IoEventParas.push({ ShapeID: eventelements[i].id, ServerID: parastr });
                }
            ...
            for (var i = 0; i < statuselements.length; i++) {
                ...
                var parastr = statuselements[i].getAttribute("data-iostatus");
                if (parastr != undefined && parastr.trim() != "") {
                    IoStatusParas.push({
                        ShapeID: statuselements[i].id,   ServerID: parastr.split(',')[0], CommunicateID: parastr.split(',')[1], DeviceID: parastr.split(',')[2]
                    });
                }
            ...
            for (var i = 0; i < trainelements.length; i++) {
                ...
                var parastr = trainelements[i].getAttribute("data-iomachinetrain");
                if (parastr != undefined && parastr.trim() != "") {
                    IoMachineTrainParas.push({
                        ShapeType:'single',  ShapeID: trainelements[i].id,  ServerID: parastr.split(',')[0], CommunicateID: parastr.split(',')[1], DeviceID: parastr.split(',')[2], TaskId: parastr.split(',')[3], ForecastPriod: parseInt(parastr.split(',')[4])
                    });
                }

            }
            for (var i = 0; i < trainlistelements.length; i++) {


                var parastr = trainlistelements[i].getAttribute("data-iomachinetrainlist");
                if (parastr != undefined && parastr.trim() != "") {
                    var arrstr = parastr.split("|");
                    for (var n = 0; n < arrstr.length; n++) {


                        IoMachineTrainParas.push({
                            ShapeType: 'list',  ShapeID: trainlistelements[i].id, ServerID: arrstr[n].split(',')[0], CommunicateID: arrstr[n].split(',')[1], DeviceID: arrstr[n].split(',')[2], TaskId: arrstr[n].split(',')[3], ForecastPriod: parseInt(arrstr[n].split(',')[4])
                        });
                    }
                }
                ...
                if (extData != undefined && extData != null) {
                    if (extData.id != undefined && extData.mapid != undefined) {
                        ...
                        if (extData.iovalue != undefined && extData.iovalue != null)
                            IoParas.push({ ShapeID: ioelements[i].id, ServerID: extData.iovalue.SERVER_ID, CommunicateID: extData.iovalue.IO_COMM_ID, DeviceID: extData.iovalue.IO_DEVICE_ID, ParaID: extData.iovalue.IO_ID, DataType: extData.iovalue.DataType, Format: extData.iovalue.Format, Unit: extData.iovalue.Unit, UpdateCycle: extData.iovalue.UpdateCycle, IoName: extData.iovalue.IoName, Value: '', DateTime: '', Status: 0, QualityStamp: 'BAD' });

                        if (extData.iovisible != undefined && extData.iovisible != null)
                            IoParas.push({ ShapeID: ioelements[i].id, ServerID: extData.iovisible.SERVER_ID, CommunicateID: extData.iovisible.IO_COMM_ID, DeviceID: extData.iovisible.IO_DEVICE_ID, ParaID: extData.iovisible.IO_ID, DataType: extData.iovisible.DataType, Format: extData.iovisible.Format, Unit: extData.iovisible.Unit, UpdateCycle: extData.iovisible.UpdateCycle, IoName: extData.iovisible.IoName, Value: '', DateTime: '', Status: 0, QualityStamp: 'BAD' });
        ...
        try {
            //获取事件信息数据
            $.ajaxSetup({ async: false });
            $.post("/Scada/ScadaFlow/GetReadEvent", { IoEventParas }, function (result) {
                //code = 0, msg = "", count = total, data = list
                for (var i = 0; i < result.count; i++) {
                    try {
                        var valueeles = document.querySelectorAll("[data-ioevent='" + result.data[i].ServerID + "']");
                        if (valueeles != undefined) {
                            for (var n = 0; n < valueeles.length; n++) {
                                var parastr = valueeles[n].getAttribute("data-ioevent");
                                SCADA.UpdateFlowEventValue(result.data[i].Items, valueeles[n].id.replace("shape", ""));
                            }
        ...
        try {
            //获取设备状态数据
            $.ajaxSetup({ async: false });
            $.post("/Scada/ScadaFlow/GetReadStatus", { IoStatusParas }, function (result) {
                //code = 0, msg = "", count = total, data = list
                for (var i = 0; i < result.count; i++) {
                    try {
                        var valueeles = document.querySelectorAll("[data-iostatus='" + result.data[i].IOStr + "']");
                        if (valueeles != undefined) {
                            for (var n = 0; n < valueeles.length; n++) {
                                var parastr = valueeles[n].getAttribute("data-iostatus");
                                SCADA.UpdateFlowStatusValue(result.data[i].Value, result.data[i].DateTime, valueeles[n].id.replace("shape", ""));
                            }
        ...
        try {
            //获取机器训练预测数据
            $.ajaxSetup({ async: false });
            $.post("/Scada/ScadaFlow/GetReadMachineTrain", { IoMachineTrainParas }, function (result) {
                for (var i = 0; i < result.count; i++) {
                    try {
                        //single
                        var valueeles = document.querySelectorAll("[data-iomachinetrain='" + result.data[i].IOStr + "']");
                        if (valueeles != undefined) {
                            for (var n = 0; n < valueeles.length; n++) {
                                var parastr = valueeles[n].getAttribute("data-iomachinetrain");
                                SCADA.UpdateFlowMachineTrainValue(result.data[i].Train,valueeles[n].id.replace("shape", ""));
                            }

                        }
                        //list
                        var valuelisteles = document.querySelectorAll("[data-iomachinetype='list']");
                      
                        if (valuelisteles != undefined) {
                            for (var n = 0; n < valuelisteles.length; n++) {
                                var parastr = valuelisteles[n].getAttribute("data-iomachinetrainlist");
                                if (parastr.includes(parastr.trim())) {//是否包含字符串，如果包含则list集合中需要增加此元素
                                    SCADA.UpdateFlowMachineTrainValue(result.data[i].Train, valuelisteles[n].id.replace("shape", ""));
                                }
...
function GetArcPath(arcRect, startAngle, sweepAngle) {
    ...
    function  GetWeek(num)
{
    ...
    ///获取一个菱形的路径
    function getDiamondPath(cp, wrc, hrc) {
    var path = "";
    path = "M" + parseFloat(cp.X - wrc / 2) + "," + cp.Y;
    path += "L" + cp.X + "," + parseFloat(cp.Y - hrc / 2);
    path += "L" + parseFloat(cp.X + wrc / 2) + "," + parseFloat(cp.Y);
    path += "L" + parseFloat(cp.X) + "," + parseFloat(cp.Y + hrc / 2);
    path += "L" + parseFloat(cp.X - wrc / 2) + "," + parseFloat(cp.Y);
    path += "Z";
    return path;

}
function getRulerBasePointorPath(Direction, ScaleDirection, rect, MajorIntervalSize, PointorRadious,x,y) {
    var path = "";
    switch (Direction) {
        case "Horizontal":
            {
                    ...
                    case "Left":
                        {
                            ...
                            path += "M" + parseFloat(x) + "," + parseFloat(y + rect.height / 2 + MajorIntervalSize + 5);
                            path += " L" + parseFloat(x - PointorRadious) + "," + parseFloat(y + rect.height / 2 + MajorIntervalSize + 5 + PointorRadious);
                            path += " L" + parseFloat(x + PointorRadious) + "," + parseFloat(y + rect.height / 2 + MajorIntervalSize + 5 + PointorRadious);
                            path += " ";
                        }
                        break;
        ...
        case "Vertical":
            {
                switch (ScaleDirection) {
                    case "Left":
                        {
                            path += "M" + parseFloat(x + rect.width / 2 - MajorIntervalSize - 5) + "," + parseFloat(y);
                            path += " L" + parseFloat(x + rect.width / 2 - MajorIntervalSize - 5 - PointorRadious) + "," + parseFloat(y - PointorRadious);
                            path += " L" + parseFloat(x + rect.width / 2 - MajorIntervalSize - 5 - PointorRadious) + "," + parseFloat(y  + PointorRadious);
                            path += " ";
...
function getRulerBasePointorLabelRect(Direction, ScaleDirection, rect, MajorIntervalSize, PointorRadious, x, y,ts) {
    var rectlabel = {};
    switch (Direction) {
        case "Horizontal":
            {
                switch (ScaleDirection) {
                    case "Left":
                        {
                            rectlabel.x = x + PointorRadious - ts.width / 2;
                            rectlabel.y = y + rect.height / 2 + MajorIntervalSize + 5 + PointorRadious + 10 + ts.height;
                            rectlabel.width = ts.width;
                            rectlabel.height = ts.height;
        ...
        case "Vertical":
            {
                switch (ScaleDirection) {
                    case "Left":
                        {
                            rectlabel.x = x + rect.width / 2 - MajorIntervalSize - 5 - PointorRadious- ts.width -10;
                            rectlabel.y = y  ;
                            rectlabel.width = ts.width;
                            rectlabel.height = ts.height;
...
/// 获取某一个线段指定距离上的点
function getLinePointFormDistance(s, e, distance) {
    var theta = Math.atan2((s.y - e.y), (s.x - e.x));
    var x2 = s.x - distance * Math.cos(theta);
    var y2 = s.y - distance * Math.sin(theta);
    var p = { x: x2, y: y2 }
    return p ;
};
...
function   getDistance(  p1,   p2)
{
    ...
    var dis = parseFloat(Math.sqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y)));
