...
namespace ScadaShapeCustum
{
    ...
    public class MyCustumShape : SVG_CommonShape
    {
        ...
        private SVG_LinearMode mLinearMode = SVG_LinearMode.垂直居中渐变;
        public SVG_LinearMode LinearMode
        {
        ...
        private SVG_FillType mFillType = SVG_FillType.颜色填充;
        public SVG_FillType FillType
        {
            set { mFillType = value; }
        ...
        public override void Paint(Graphics g)
        {
            ...
            using (GraphicsPath path = base.GetRoundRectPath(this.Rectangle,10))
            {
                ...
                else if (FillType == SVG_FillType.渐进填充)
                {
                ...
                if (this.Site.IsPublish)
                {
                    ...
                    else if (FillType == SVG_FillType.不填充)
                    {
                        fill = "fill='none'";
                    }
                    ...
                    string penlinedefs = ""; 
                    string strokstyle = Border.GetSVGStroke(GUIDToNormalID.GuidToLongID().ToString(), out penlinedefs);//获取线的SVG格式
            ...
            this.SvgManager.Add(svg_str.ToString());
        ...
        public override void AddProperties()
        {
            ...
            Bag.Properties.Add(new PropertySpec("填充类型", typeof(SVG_FillType), "图形属性", "包含渐进填充和颜色填充两种", this.FillType));
            Bag.Properties.Add(new PropertySpec("渐变类型", typeof(SVG_LinearMode), "图形属性", "主要是渐变方向，水平，垂直，中心放射", this.mLinearMode));
        ...
        protected override void SetPropertyBagValue(object sender, PropertySpecEventArgs e)
        {
            ...
            {
                case "渐变类型":
                    this.mLinearMode = (SVG_LinearMode)e.Value; break;
                case "前景色":
                    this.mFore = (SVG_Color)e.Value; break;
                case "填充色":
                    this.mBack = (SVG_Color)e.Value; break;
                case "填充类型":
                    this.FillType = (SVG_FillType)e.Value;
                    break;
                case "画笔":
        ...
        protected MyCustumShape(SerializationInfo info, StreamingContext context) : base(info, context)
        {
            ...
            #region 自定义属性
            this.mFillType = (SVG_FillType)info.GetValue("mFillType", typeof(SVG_FillType));
            this.mFore = (SVG_Color)info.GetValue("mFore", typeof(SVG_Color));
            this.mBack = (SVG_Color)info.GetValue("mBack", typeof(SVG_Color));
            this.mLinearMode = (SVG_LinearMode)info.GetValue("mLinearMode", typeof(SVG_LinearMode));
        ...
        {
            MessageBox.Show("是否要删除此图元");
            base.Delete();
        }
        public override void Copy()
        {
            MessageBox.Show("是否要复制此图元");
            base.Copy();
        }
        ...
        public override void SetCursor(Cursor cur)
        {
            base.SetCursor(cur);
        }
        public override void SetStatBar(ToolStripStatusLabel l)
        {
            base.SetStatBar(l);
        }
        public override void SetToolTip(ToolTip tip)
        {
            base.SetToolTip(tip);
        }
    }
