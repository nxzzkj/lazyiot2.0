...
namespace Modbus
{
    ...
    public abstract class ModbusConvert
    {
        ...
        /// <returns></returns>
      public static int Set_Int_bit(int data, int index, bool flag)
        {
            if (flag)
            {
                data |= (0x1 << index);
        ...
        public static int Get_Int_bit(byte input, int index)
        {
    ...
    public class ModbusFragmentStore
    {
        ...
        public void MakeFragment()
        {
            ...
            if (FixUnit == 0)//表示对寄存器读取分段处理
            {
                ...
                if(Units.Count>0)
                {
                    max = Units[Units.Count - 1];
                }
                if(max- min< maxnum)
                {
                    ModbusFragment fragment = new ModbusFragment();
                    fragment.Code = this.StoredCode;
                    fragment.StartRegister = Convert.ToUInt16(min);
                    fragment.DevAddress = DevAddress;
                    fragment.RegisterNum = Convert.ToUInt16(max - min);
                    Fragments.Add(fragment);

                }
                else
                {      //分段处理
                    int fi = (max - min) / maxnum;
                    int num = (max - min) % maxnum;
                    for(int i=0;i<fi;i++)
                    {
                        ModbusFragment fragment = new ModbusFragment();
                        fragment.Code = this.StoredCode;
                        fragment.StartRegister = Convert.ToUInt16(min+i * maxnum);
                        fragment.DevAddress = DevAddress;
                        fragment.RegisterNum = Convert.ToUInt16(maxnum);
                        Fragments.Add(fragment);
                    }
                    if(num!=0)
                    {
                        ModbusFragment fragment = new ModbusFragment();
                        fragment.Code = this.StoredCode;
                        fragment.StartRegister = Convert.ToUInt16(min+fi * maxnum);
                        fragment.DevAddress = DevAddress;
                        fragment.RegisterNum = Convert.ToUInt16(num+4);
                        Fragments.Add(fragment);
                    }
