namespace Modbus.Utility
{
    ...
    public static class ModbusUtility
    {
        ...
        public static List<string> GetLocalAllIpAddress()
        {
            List<string> ips = new List<string>();
            string hostName = Dns.GetHostName();   //获取本机名
            IPHostEntry localhost = Dns.GetHostEntry(hostName);    //方法已过期，可以获取IPv4的地址
              foreach(IPAddress ip in localhost.AddressList)                                  //IPHostEntry localhost = Dns.GetHostEntry(hostName);   //获取IPv6地址
            {
                ips.Add(ip.MapToIPv4().ToString());
            }
