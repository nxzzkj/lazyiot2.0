...
namespace Modbus.ModbusService
{
    ...
    public class Modbus_Serial_IP_Device_PARA
    {
    ...
    public class ModbusBusSerialIPObject
    {
   
        public RealData RealData { set; get; }
        public IO_DEVICE Device { set; get; }

        public Modbus_Serial_IP_Device_PARA Para { set; get; }
    ...
    /// Modbus TCP 通讯
    /// </summary>
    public class Modbus_Serial_IP : ScadaCommunicateKernel
    {
  
        private const string mGuid = "BC24040E-2061-4CE3-9A40-03691A765F0A";
        /// <summary>
        /// 驱动唯一标识，采用系统GUID分配
        /// </summary>
        ...
        private string mTitle = " Modbus 串口转TCP/UDP";
        ...
        protected override bool InitCommunicateKernel(IO_SERVER server, IO_COMMUNICATION communication, List<IO_DEVICE> ioDevices, SCADA_DRIVER driver)
        {
            ...
            busTcpObjects = new List<ModbusBusSerialIPObject>();
            ...
            if (communication != null)
            {
                ...
                for (int i = 0; i < this.IODevices.Count; i++)
                {
                    ...
                    ModbusBusSerialIPObject modbusBusTcp = new ModbusBusSerialIPObject();
                    modbusBusTcp.Para = new Modbus_Serial_IP_Device_PARA();
        ...
        private bool RequestData(IO_DEVICE device, ModbusBusSerialIPObject rData, out string error, ModbusFragmentStore fragmentstore)
        {
        ...
        private bool ResponseData(IO_SERVER server, IO_COMMUNICATION comm, IO_DEVICE device, IO_PARA para, string value, ModbusBusSerialIPObject rData)
        {
            ModbusSerialMaster master = rData.Master;
            if (para == null)
                return false;
        ...
        protected override ScadaResult IOSendCommand(IO_SERVER server, IO_COMMUNICATION comm, IO_DEVICE device, IO_PARA para, string value)
        {
            ...
            if (device == null)
                return res;
            ModbusBusSerialIPObject modbusTcp = busTcpObjects.Find(x => x.Device.IO_DEVICE_ID == device.IO_DEVICE_ID);
            ...
            return Task.Run(() =>
            {
                ...
                for (int i = 0; i < this.busTcpObjects.Count; i++)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(mRealData.Device.IO_DEVICE_PARASTRING))
                        continue;
                    ModbusBusSerialIPObject modbusBusTcp = this.busTcpObjects[i];
                    if (Tcp_PARA.CommunicateMode == ModbusTCP_CommunicateMode.TCP)
                    {
                        try
                        {
                    ...
                    else
                    {
                        try
                        {
                            ...
                            if (busTcpObjects[i].Para.ModbusType == "RTU")
                            {
                                ModbusSerialMaster master = ModbusSerialMaster.CreateRtu(udplient);
                                master.Transport.Retries = modbusBusTcp.Para.CollectFaultsNumber;//重试次数
                                master.Transport.WaitToRetryMilliseconds = modbusBusTcp.Para.CollectFaultsInternal;//重试间隔     
                                modbusBusTcp.Master = master;
                            ...
                            {
                                ModbusSerialMaster master = ModbusSerialMaster.CreateAscii(udplient);
                                master.Transport.Retries = modbusBusTcp.Para.CollectFaultsNumber;//重试次数
                                master.Transport.WaitToRetryMilliseconds = modbusBusTcp.Para.CollectFaultsInternal;//重试间隔     
                                modbusBusTcp.Master = master;
